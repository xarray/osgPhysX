...
#else
    for ( unsigned int i=0; i<_vehicleEngines.size(); ++i )
        _numTotalWheels += _vehicleEngines[i]->mWheelsSimData.getNumWheels();
#endif
}
...
#else
            if ( _vehicleEngines.size()>0 )
                VehicleManager::instance()->update( s, _sceneName, _vehicleEngines, _numTotalWheels );
#endif
...
#else
        if ( _vehicleEngines.size()>0 )
            VehicleManager::instance()->update( step, _sceneName, _vehicleEngines, _numTotalWheels );
#endif
